    ...
    getCurrentLevelSet() {
      return {
        ...
        getCurrentLevel: () => ({
          getId: () => 'level-1',
          getName: () => 'Test Level',
          next: () => ({ getId: () => 'level-2' }) // Has next level
        })
      };
    }
  }
...
describe('LevelOver Scene Event Cleanup', () => {
  let scene: LevelOver;
  let mockScene: any;
  ...
  describe('button listener removal', () => {
    it('should remove all nextLevelButton listeners on shutdown', () => {
      scene.create();

      const nextLevelButton = scene.nextLevelButton as any;
      expect(nextLevelButton.listenerCount('pointerdown')).toBeGreaterThan(0);
      expect(nextLevelButton.listenerCount('pointerover')).toBeGreaterThan(0);
      expect(nextLevelButton.listenerCount('pointerout')).toBeGreaterThan(0);

      scene.shutdown();

      expect(nextLevelButton.listenerCount('pointerdown')).toBe(0);
      expect(nextLevelButton.listenerCount('pointerover')).toBe(0);
      expect(nextLevelButton.listenerCount('pointerout')).toBe(0);
    });
